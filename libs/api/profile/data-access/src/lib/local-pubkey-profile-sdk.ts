...
export class LocalPubKeyProfileSdk {
  ...
  createProfile({ authority, avatarUrl, feePayer, username }: CreateProfileOptions) {
    const [profile] = this.getProfilePda({ username })
    const [pointer] = this.getPointerPda({ provider: PubKeyIdentityProvider.Solana, providerId: authority.toString() })
    return this.program.methods.createProfile({ avatarUrl, username }).accountsStrict({
      authority,
      feePayer: feePayer.publicKey,
      pointer,
      profile,
